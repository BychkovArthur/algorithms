...
public:
    ...
    std::pair<bool, uint64_t> findd(const std::string& str);
};
...
bool RB::insert(const std::pair<std::string, uint64_t>& val) {
    ...
    if (!place.first) {
        place.first = new Node(std::move(val), place.second);
        ++sz;
...
std::pair<bool, uint64_t> RB::findd(const std::string& str) {
    std::pair<Node*&, Node*> place = find({str, 0ULL});
    ...
    return {true, place.first->val.second};
}
...
int main() {
    ...
    while (fin >> input1) {
        ...
        input.push_back({{input1, input2}, input3});
    }
    ...
    for (size_t i = 0; i < input.size(); ++i) {
        ...
        if (input1.size() == 1 && input1[0] == '+') {
            ...
            lower(input1);
            if (auto res = mp1.findd(input1); res.first) {
                outpu1 << "OK: " << res.second << '\n';
            } else {
                outpu1 << "NoSuchWord" << '\n';
            }
        }
