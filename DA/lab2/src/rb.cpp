...
/*
    ...
    8) переписать на шаблоны
    9) лобавить сериализацию
    10) надо чтобы пользователь реализовал сериализатор для ключ/значение. тогда будет работать мой сериализатор
    11) Убрать выброс исключений в operator[], это очень медленно.
