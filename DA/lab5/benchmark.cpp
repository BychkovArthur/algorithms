...
std::vector<std::size_t> naive(const std::string& text, const std::string& pattern) {
    std::vector<std::size_t> ans;
    for (ssize_t i = 0; i <= text.size() - pattern.size(); ++i) {
        bool ok = true;
        for (ssize_t j = 0; j < pattern.size(); ++j) {
...
int main() {
    ...
    std::ofstream sa_out("suffix_array_output.txt");
    std::ofstream naive_out("naive_output.txt");
    ...
    for (const auto& pattern : patterns) {
        const auto& ids = sa.find_all_super_fast(pattern);
    ...
    i = 0;
    for (const auto& pattern : patterns) {
        const auto& ids = naive(text, pattern);
        if (ids.empty()) continue;
        naive_out << ++i << ": " << ids << '\n';
    }
    auto end_naive = std::chrono::steady_clock::now();

    std::cout << "SuffixArray: " << std::chrono::duration_cast<std::chrono::milliseconds>(end_sa - start_sa).count() << " ms" << std::endl;
    std::cout << "Naive: " << std::chrono::duration_cast<std::chrono::milliseconds>(end_naive - start_naive).count() << " ms" << std::endl;
