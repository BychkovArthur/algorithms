...
public:
    ...
    SuffixArrayNaive(const std::string& text) : text_(text), n_(text_.size() + 1), array_(n_, -1) {
        build();
    }
...
public:
    ...
    SuffixArrayFast(const std::string& text) : text_(text), n_(text_.size() + 1), array_(n_, -1), neighbor_lcp_(n_ - 1) {
        build();
        kasai();
    }
...
public:
    ...
    SuffixArraySuperFast(const std::string& text) : text_(text), n_(text_.size() + 1), array_(n_, -1), neighbor_lcp_(n_ - 1), st_() {
        build();
        kasai();
...
format: text_len pattern_len pattern_cnt alphabet

1000000 5 10000 abcdef
Naive: 1335 ms
Fast : 1381 ms
SFast: 1504 ms

1000000 5 100000 abcdef
Naive: 5473 ms
Fast : 4792 ms
SFast: 5889 ms

1000000 10 100000 abc
Naive: 1933 ms
Fast : 2009 ms
SFast: 2936 ms
...
int main() {
    ...
    std::ofstream sa_naive_out("suffix_array_naive_output.txt");
    std::ofstream sa_fast_out("suffix_array_fast_output.txt");
    std::ofstream sa_super_fast_out("suffix_array_super_fast_output.txt");
    ...
    auto start_sa_naive = std::chrono::steady_clock::now();
    SuffixArrayNaive sa1(text);
    std::size_t i = 0;
    for (const auto& pattern : patterns) {
        // std::cout << i << '\n';
        const auto& ids = sa1.find_all(pattern);
        if (ids.empty()) continue;
        sa_naive_out << ++i << ": " << ids << '\n';
    }
    auto end_sa_naive = std::chrono::steady_clock::now();

    auto start_sa_fast = std::chrono::steady_clock::now();
    SuffixArrayFast sa2(text);
    i = 0;
    for (const auto& pattern : patterns) {
        const auto& ids = sa2.find_all_fast(pattern);
        if (ids.empty()) continue;
        sa_fast_out << ++i << ": " << ids << '\n';
    }
    auto end_sa_fast = std::chrono::steady_clock::now();

    auto start_sa_super_fast = std::chrono::steady_clock::now();
    SuffixArraySuperFast sa3(text);
    i = 0;
    for (const auto& pattern : patterns) {
        const auto& ids = sa3.find_all_super_fast(pattern);
        if (ids.empty()) continue;
        sa_super_fast_out << ++i << ": " << ids << '\n';
    }
    auto end_sa_super_fast = std::chrono::steady_clock::now();

    std::cout << "Naive: " << std::chrono::duration_cast<std::chrono::milliseconds>(end_sa_naive - start_sa_naive).count() << " ms" << std::endl;
    std::cout << "Fast : " << std::chrono::duration_cast<std::chrono::milliseconds>(end_sa_fast - start_sa_fast).count() << " ms" << std::endl;
    std::cout << "SFast: " << std::chrono::duration_cast<std::chrono::milliseconds>(end_sa_super_fast - start_sa_super_fast).count() << " ms" << std::endl;
}