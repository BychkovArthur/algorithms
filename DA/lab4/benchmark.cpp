...
void read(std::vector<uint32_t>& text, std::vector<uint32_t>& pattern, std::vector<size_t>& lines_length) {
    std::ifstream in("benchmark_input.txt");
...
void write_answer(std::vector<ssize_t>& ans, std::ofstream& ofs, std::vector<size_t>& lines_length) {
    ...
    for (size_t i = 0; i < lines_length.size(); ++i) {
        ...
        while (current_ans_ind < ans.size() && ans[current_ans_ind] <= words_before_current_line_include_current_line) {
            ofs << i + 1 << ", " << ans[current_ans_ind++] - words_before_current_line_exclude_current_line << '\n';
        }
...
int main() {
    ...
    std::ofstream out_ag("benchmark_out_ag.txt");
    std::ofstream out_find("benchmark_out_find.txt");
    std::vector<ssize_t> ans_ag;
    std::vector<ssize_t> ans_find;

    read(text, pattern, lines_length);

    auto start_ag = std::chrono::steady_clock::now();
    ApostolicoGiancarlo ag(std::move(text), std::move(pattern));
    auto end_ag = std::chrono::steady_clock::now();

    ans_ag = ag.get_answer();
    for (auto& el : ans_ag) ++el; // Переведём всё в 1-индексацию
    write_answer(ans_ag, out_ag, lines_length);

    auto start_search = std::chrono::steady_clock::now();
    auto current_start_it = text.begin();
    auto curr_it = std::search(current_start_it, text.end(), pattern.begin(), pattern.end());
    while (curr_it != text.end()) {
        ans_find.push_back(curr_it - text.begin() + 1);
        current_start_it = curr_it + 1;
        curr_it = std::search(current_start_it, text.end(), std::boyer_moore_searcher(pattern.begin(), pattern.end()));
    }
    auto end_search = std::chrono::steady_clock::now();
    write_answer(ans_find, out_find, lines_length);


    std::cout << "Apostolico-Giancarlo " << std::chrono::duration_cast<std::chrono::milliseconds>(end_ag - start_ag).count() << "ms" << std::endl;
    std::cout << "std::search " << std::chrono::duration_cast<std::chrono::milliseconds>(end_search - start_search).count() << "ms" << std::endl;
