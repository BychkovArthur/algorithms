...
std::vector<std::uint64_t> generateValues(std::size_t size,
    ...
    // Важно! В counting_sort нельзя задавать большой диапозон
    // Из-за большого диапазона будет создаваться большой массив -> не хватит памяти
...
int main(int argc, char* argv[]) {
    ...
    const BenchmarkResult userResult =
        runBenchmark(CountingSort, source, expected, repetitions);
